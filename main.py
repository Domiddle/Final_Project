...
if __name__=="__main__":
    ...
    Roberto_Carlos=EAFC25_cards("Roberto Carlos","TOTY Icon",92,92,87,88,85,90,91)
...
class HigherLowerGUI:
    def __init__(self, master):
        ...
        self.player1_button = tk.Button(master, text="", font=("Helvetica", 14), width=20, command=lambda: self.check_guess(1))
        self.player1_button.pack(pady=5)

        self.player2_button = tk.Button(master, text="", font=("Helvetica", 14), width=20, command=lambda: self.check_guess(2))
    ...
    def next_round(self):
        ...
        self.player1, self.player2 = random.sample(all_players, 2)
    ...
    def check_guess(self, guess):
        ...
        correct = (p1_stat > p2_stat and guess == 1) or (p2_stat > p1_stat and guess == 2) or (p1_stat == p2_stat and guess == 0)

        if correct:
            self.score += 1
            if self.score > self.high_score:
                self.high_score = self.score
            messagebox.showinfo("Correct!", f"Correct!\n{self.player1.name}: {p1_stat}\n{self.player2.name}: {p2_stat}\nYour score: {self.score}")
            self.next_round()
        else:
